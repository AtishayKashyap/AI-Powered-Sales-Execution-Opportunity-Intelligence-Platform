...
flowchart TD
    ...
    E --> F[Evidence-Grounded Explanation]
    F --> G[React Sales Copilot]
...
SalesForge deliberately separates **business logic** from **language generation**.
...
This makes the system easier to audit and allows the application to continue operating with a deterministic explanation fallback when an external AI provider is unavailable.
...
The top-action queue is designed to turn a large opportunity universe into a manageable field-execution workload.
...
The AI explanation receives structured opportunity information such as:
...
The system is designed to avoid allowing the AI layer to invent numeric business evidence.

### Deterministic fallback

The application also contains a deterministic explanation fallback.

If an AI provider is unavailable, the system can still present an evidence-grounded explanation generated from the structured opportunity data.

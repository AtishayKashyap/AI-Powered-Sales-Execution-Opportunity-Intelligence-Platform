...
def copy_base_data():
    """
    Copy the existing v1 generated dataset into generated_v2.
    We then apply controlled scenario modifications.
    """
...
def choose_scenarios(outlets):
    ...
    Keeping the groups mostly disjoint makes evaluation much easier.
    """
...
def modify_revenue_decline(
    ...
    We do this by reducing the value of recent orders and, for some
    orders, removing them entirely.
    """
    ...
    remove_mask = (
        current_orders["order_id"].isin(
            current_orders.sample(
                frac=0.35,
                random_state=SEED,
            )["order_id"]
        )
...
def modify_inactive(
    ...
    Recent orders are removed, while historical activity remains.
    """
    ...
    # Keep some visits so the rep can still have attempted contact.
    recent_visit_mask = (
...
def modify_stock_risk(
    ...
    # Other snapshots have low inventory.
    remaining_index = (
...
def create_cross_sell_ground_truth(
    ...
    """
    Create a deterministic cross-sell ground truth.

    For each selected outlet:
      - identify its most purchased category
      - identify a category commonly purchased by similar
        outlets in its territory
      - mark the second category as an assortment gap

    We do not artificially add orders here. The opportunity is
    defined as a category-level gap.
    """

    # order_items does not contain outlet_id directly.
    # Recover the relationship through orders.
...
def main():
    ...
    print("=" * 70)
    print("SALESFORGE — SCENARIO GENERATOR V2")
    print("=" * 70)
    ...
    print("\nScenario generator v2 complete.")
